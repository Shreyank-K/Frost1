...
function weightedPercentile(values, weights = [], percentile = 0.5) {
  if (!values.length) return 0;
...
function normalizeCondition(raw) {
  ...
  if (condition.includes('seller refurbished') || condition.includes('manufacturer refurbished') || condition.includes('certified refurbished') || condition.includes('refurbished')) return 'refurbished';
...
function detectListingType(buyingOptions = []) {
...
function scoreListingType(listingType) {
...
function scoreTitleSimilarity(query, title) {
...
function scoreConditionSimilarity(targetCondition, compCondition) {
...
function selectRelevantComparables(items) {
  if (!items.length) {
...
function calculateConfidence(input, rawCount = 0) {
...
function analyzeComparablePricing(comparables, options = {}) {
...
function estimateResalePrice(comps, options = {}) {
  const analysis = analyzeComparablePricing(comps, options);
  return {
...
function estimateFees(sellPrice) {
  const feeRate = 0.1325;
  const perOrderFee = 0.30;
  return round2(sellPrice * feeRate + perOrderFee);
}

function estimateShipping(sellPrice) {
  if (sellPrice < 20) return 4.50;
  if (sellPrice < 50) return 6.50;
  if (sellPrice < 100) return 8.50;
  if (sellPrice < 200) return 10.50;
  return 13.50;
}

function estimateBreakEvenPrice(buyPrice) {
  const safeBuyPrice = Math.max(0, Number(buyPrice) || 0);
  if (!safeBuyPrice) return 0;

  const profitAtPrice = (sellPrice) => sellPrice - safeBuyPrice - estimateFees(sellPrice) - estimateShipping(sellPrice);
